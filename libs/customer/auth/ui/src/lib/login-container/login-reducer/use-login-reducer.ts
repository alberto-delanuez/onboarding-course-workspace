...
import { ActionHandler, LoginAction, LoginState } from "./reducer.types";
...
export const useLoginReducer = (initialState: LoginState) => {
  const actionHandlers: Record<LoginAction['type'], ActionHandler> = {
    ...
    SET_EMAIL: (state, action) => ({ 
      ...state, 
      email: (action as Extract<LoginAction, { type: 'SET_EMAIL' }>).payload, 
      error: null 
    }),
    SET_CODE: (state, action) => ({ 
      ...state, 
      code: (action as Extract<LoginAction, { type: 'SET_CODE' }>).payload, 
      error: null 
    }),
    ...
    SET_ERROR: (state, action) => ({ 
      ...state, 
      error: (action as Extract<LoginAction, { type: 'SET_ERROR' }>).payload 
    }),
    CLEAR_ERROR: (state) => ({
      ...state,
      error: null
