...
import {
    AuthHttpRepository,
    authClient
} from '@onboarding-course/customer-auth-infrastructure';
import {
    GetProfileUseCase,
...
export type LoginConfig = { enabled: false } | { enabled: true };
...
export class CustomerAppBuilder {
    ...
    public withSocialLogin(
        providers: ('google' | 'microsoft' | 'apple' | 'github')[] = ['google']
    ): CustomerAppBuilder {
    ...
    public withAuthModule(): CustomerAppBuilder {
        const authRepo = new AuthHttpRepository(authClient);
        ...
        DIContainer.set(
            SocialLoginUseCaseToken,
            new SocialLoginUseCase(authRepo)
        );
        DIContainer.set(
            RequestOtpUseCaseToken,
            new RequestOtpUseCase(authRepo)
        );
        DIContainer.set(
            LoginWithOtpUseCaseToken,
            new LoginWithOtpUseCase(authRepo)
        );
    ...
    public withProfileModule(): CustomerAppBuilder {
        ...
        DIContainer.set(
            GetProfileUseCaseToken,
            new GetProfileUseCase(profileRepo)
        );
        DIContainer.set(
            UpdateProfileUseCaseToken,
